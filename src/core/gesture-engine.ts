...
function eyeRatio(points: FacePoint[]) {
...
export function inferGesture(params: {
  ...
  const now = Date.now()
  if (now - lastTriggeredAt < config.cooldownMs || landmarks.length < 468) return null
  ...
  if (leftRatio < config.winkThreshold && rightRatio > config.winkThreshold + 0.04) {
    return { label: 'Left wink', action: config.leftWinkAction, confidence: Math.max(0, 1 - leftRatio / config.winkThreshold), timestamp: now }
  }
  if (rightRatio < config.winkThreshold && leftRatio > config.winkThreshold + 0.04) {
    return { label: 'Right wink', action: config.rightWinkAction, confidence: Math.max(0, 1 - rightRatio / config.winkThreshold), timestamp: now }
  }
  if (normalizedYaw > config.yawThreshold) {
    return { label: 'Head right', action: 'next', confidence: Math.min(1, normalizedYaw / (config.yawThreshold * 1.8)), timestamp: now }
  }
  if (normalizedYaw < -config.yawThreshold) {
    return { label: 'Head left', action: 'prev', confidence: Math.min(1, Math.abs(normalizedYaw) / (config.yawThreshold * 1.8)), timestamp: now }
  }
  return null
}
